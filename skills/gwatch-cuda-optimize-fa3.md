# FA3 Auto Optimization Skill (G-Watch + FlashAttention3 Hopper)
This skill teaches the agent to profile and optimize FlashAttention3 (FA3) kernels on Hopper using G-Watch.


## Goal
Your primary objective is to iteratively optimize the FlashAttention-3 (FA3) kernel and significantly improve its **TFLOPs** performance. 
To achieve this,
...
- run range profiling, PC sampling, intra-kernel tracing or conduct static binary analysis of FA3 kernels.
- modify FA3 source code and rebuild.
- iterate profiling until bottlenecks are reduced.

Target scripts:
- `examples/cuda/profiler/fa3/do_flops_fa3.py`
- `examples/cuda/profiler/fa3/do_range_profile_fa3.py`
- `examples/cuda/profiler/fa3/do_pc_sampling_fa3.py`
...
```bash
python3 examples/cuda/profiler/fa3/do_flops_fa3.py \
    --device-id 0 \
    --batch 8 \
    --seqlen 8192 \
    --nheads 16 \
...
For GQA sweeps, append `--model-config examples/cuda/model_config/llama4.json` (or point to `qwen3.5.json` / `qwen3.json`), or override via `--q-heads`/`--kv-heads`.
...
```bash
gwatch profile show-topo --chip h100
```

Then list candidates, for instance:

```bash
gwatch profile list-metrics --chip h100 --unit smsp --subunit sass
```
...
```bash
gwatch profile show-metric-details --chip h100 --name <metric_base_name>
# or on some versions:
gwatch profile show-metric-details --chip h100 --name <metric_base_name>
```
...
Use `--model-config examples/cuda/profiler/fa3/config/qwen3.5.json` to profile that GQA shape, or override `--q-heads`/`--kv-heads` manually.
...
```bash
gwatch show /tmp/fa3_range_profile.gwatch --mode range_profile \
...
```bash
gwatch profile python3 examples/cuda/profiler/fa3/do_pc_sampling_fa3.py \
    --device-id 0 \
    --batch 8 \
    --seqlen 8192 \
    --nheads 16 \
...
Set `--model-config examples/cuda/profiler/fa3/config/qwen3.json` to reuse that config's head ratio.

### View PC Sampling Results
Inspect results of the PC sampling using `gwatch show`:

```bash
gwatch show /tmp/fa3_pc_sampling.gwatch --mode pc_sampling \
  --kernel ".*flash.*" \ # Optional: Filter by kernel name regex
...
you can trace a specific region in the FA3 source code that doesn't have markers by adding them manually:

1. **Include Header**: Ensure `#include "gwatch/cuda/trace.hpp"` is present in the target `.h` or `.hpp` file.
...
3. **Map Names**: Add your `TAG_ID` and a descriptive name to `name_id_map` in `examples/cuda/profiler/fa3/do_trace_fa3.py`.
4. **Rebuild**: Follow the rebuild steps below to apply changes.

Example from `mainloop_fwd_sm90_tma_gmma_ws.hpp`:
```cpp
...
```bash
gwatch profile python3 examples/cuda/profiler/fa3/do_trace_fa3.py \
    --device-id 0 \
    --batch 8 \
    --seqlen 8192 \
    --nheads 16 \
...
Note that `do_trace_fa3.py` internally uses a `name_id_map` to map numeric scope IDs to human-readable names (e.g., `producer_load_K`).
...
After running `do_trace_fa3.py`, inspect the trace results using `gwatch show` in `trace` mode.

```bash
gwatch show /tmp/fa3_trace.gwatch --mode trace \
  --tid "[0,31]" \ # Optional: filter by thread IDs
  --longest-bubble \ # Optional: highlight the longest gap between events
  --longest "producer_load_K" \ # Optional: find the longest instance of a specific event
...
Expected to get: FA3 kernel instance, SASS instruction details, etc.

Use `ProfileContext` runtime-control APIs inside a `gwatch profile` run to inspect loaded FA3 kernels:
...
# run FA3 workload first so kernels are loaded
run_fa3_once()
...
for mangled, kdef in kernel_map.items():
    ...
    if re.search(r"flash|FlashAttn|sm90", mangled) or re.search(r"flash|FlashAttn|sm90", demangled):
        fa3_items.append((mangled, demangled, kdef))

print("fa3 kernels:", len(fa3_items))

if fa3_items:
    mangled, demangled, kdef = fa3_items[0]
...
Use this to:
- fetch runtime-loaded FA3 `KernelDefSASS`,
...
- **Tool 1 (Baseline FLOPs):** Use this to measure the macroscopic throughput (TFLOPS) of the FA3 kernel. This is your ultimate source of truth for validating whether a code change yielded an actual performance improvement.
...
- **Tool 4 (Intra-Kernel Trace):** Utilize this powerful tracing tool to observe thread-block and warp-level execution timelines. This is exceptionally critical for understanding the complex synchronization and software pipelining behaviors inherent to FA3's warp-specialized design.
...
1. Benchmarking TFLOPs of the original baseline implementation (via `do_flops_fa3.py`).
2. Profile the implementation via tools mentioned above (range_profile/pc_sampling/trace).
3. Form one concrete hypothesis (for example, reduce memory pressure in mainloop or reduce epilogue overhead).
...
- Attempting runtime kernel lookup before FA3 kernels are actually launched.
...
4. **Modified FA3 Source Code:**
   - Provide the complete, final optimized source code (or a comprehensive unified `diff`/patch) for all modified FA3 files.
