...
# Import the triton FA2 attention from test.py in the same directory.
...
def _flops(batch, nheads, seqlen_q, seqlen_k, headdim, causal=False):
    if causal:
        ...
        avg_seqlen = seqlen_k
    return batch * nheads * 2 * seqlen_q * avg_seqlen * (headdim + headdim)
...
def main():
    parser = argparse.ArgumentParser(description="Measure Triton FlashAttention2 FLOPs and throughput.")
    ...
    parser.add_argument("--kv-heads", type=int, default=None, help="Number of key/value heads (triton FA2 uses same for Q/K/V).")
    parser.add_argument(
        "--model-config",
        type=str,
        default=None,
    ...
    parser.add_argument("--sm-scale", type=float, default=1.0, help="Softmax scale factor.")
    ...
    # Triton FA2 uses layout (batch, heads, seqlen, headdim) and same heads for Q/K/V.
    # For GQA, we use q_heads for the benchmark (kv_heads is informational).
    nheads = q_heads
