...
# Default kernel regex for Triton FA2 backward kernels
fa2_triton_bwd_kernel_regex = ".*_attn_bwd.*"

# Backward kernel scope name_id_map (logical phases).
name_id_map = {
    200: "gemm_qk",
    210: "softmax_bwd",
    220: "gemm_dp",
...
def main():
    parser = argparse.ArgumentParser(description="Trace Triton FA2 backward kernels with do_trace.")
    ...
    parser.add_argument("--kernel-regex", type=str, default=fa2_triton_bwd_kernel_regex)
    parser.add_argument("--warmup", type=int, default=25)
    parser.add_argument("--rep", type=int, default=100)
    parser.add_argument("--dump-path", type=str, default="./fa2_triton_bwd_trace.gwatch")
    ...
    if len(results) == 0:
        summary = {
            "trace_results": 0,
            "note": "no trace results returned (Triton backward kernel may not contain trace hints)",
            "dump_path": args.dump_path,
        }
