...
def _build_inputs(batch, seqlen, q_heads, kv_heads, headdim, dtype, device):
    q = torch.randn(batch, seqlen, q_heads, headdim, device=device, dtype=dtype, requires_grad=True)
    k = torch.randn(batch, seqlen, kv_heads, headdim, device=device, dtype=dtype, requires_grad=True)
    v = torch.randn(batch, seqlen, kv_heads, headdim, device=device, dtype=dtype, requires_grad=True)
    return q, k, v


def main():
    parser = argparse.ArgumentParser(description="Range profile FlashAttention2 backward kernel with do_range_profile.")
    ...
    parser.add_argument(
        "--dump-path",
        type=str,
        default="./fa2_bwd_range_profile.gwatch",
