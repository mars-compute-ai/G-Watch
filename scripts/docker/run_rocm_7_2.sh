...
print_usage() {
    echo ">>>>>>>>>> G-Watch Container Starter (ROCm) <<<<<<<<<<"
...
start_container() {
    ...
    container_name=gw_${user_name}_rocm_7_2_$container_id
    ...
    if [ $mount = false ] ; then
        ...
        cd $script_dir && cd .. && cd ..
        # note: we copy files except the [xxx] folder, which we mount to the container
        # while read line; do $SUDO docker cp $line $container_name:/root; done < <(find . -mindepth 1 -maxdepth 1 | grep -v "examples$")
    else
        cd $script_dir && cd .. && cd ..
        $SUDO docker run -dit \
                    --device=/dev/kfd --device=/dev/dri --group-add=video \
